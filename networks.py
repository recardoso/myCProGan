...
class DenseLReq(Dense):
    ...
    def __init__(self, input_shape_value, fan_in, *args, **kwargs):
    ...
    def build(self, input_shape):
        ...
        gain=np.sqrt(2)
        std = gain / np.sqrt(self.fan_in) # He init
        self.wscale = std
...
def Generator_model(resolution=256,num_channels=3): #confirm params and confirm initializers strides bias
    resolution_log2 = int(np.log2(resolution))
    dshape=( 3, 256, 256) 
    ...
    # Encoder ---------------------------------------------------------------------------------------------------------
    # convert from rgb ------------------------------------
    x = Conv2DLReq(latent.shape[1].value, None, normal_gain, num_channels, kernel_size=1, strides=(1,1), data_format='channels_first',padding='same', bias_initializer='zeros')(latent)
    ...
    for res in range(resolution_log2, 4, -1):
        ...
        x = Conv2DLReq(x.shape[1].value, None, normal_gain, 128, kernel_size=3, strides=(1,1), data_format='channels_first',padding='same', bias_initializer='zeros')(x)
    ...
    x = Conv2DLReq(x.shape[1].value, None, normal_gain, 32, kernel_size=3, strides=(1,1), data_format='channels_first',padding='same', bias_initializer='zeros')(x)
    x = AveragePooling2D(pool_size=(2, 2), strides=(2, 2), padding='valid', data_format='channels_first')(x)
    x = LeakyReLU()(x)
    # -----------------------------------------------------
    # Dense -----------------------------------------------
    x = tf.reshape(x, [-1, np.prod([d.value for d in x.shape[1:]])])
    x = DenseLReq(x.shape[1].value, None, 2048, bias_initializer='zeros')(x)
    x = LeakyReLU()(x)
    # -----------------------------------------------------
    # Pixel Normalization ---------------------------------
    # add bias
    ...
    x = Conv2DLReq(x.shape[1].value, None, normal_gain, 64, kernel_size=3, strides=(1,1), data_format='channels_first',padding='same', bias_initializer='zeros')(x)
    x = LeakyReLU()(x)
    x = x * tf.rsqrt(tf.reduce_mean(tf.square(x), axis=1, keepdims=True) + 1e-8) # Pixel Normalization
    # -----------------------------------------------------
    # to rgb ----------------------------------------------
    images_out = Conv2DLReq(x.shape[1].value, None, 1 , num_channels, kernel_size=1, strides=(1,1), data_format='channels_first',padding='same', bias_initializer='zeros')(x)
    ...
    for res in range(6, resolution_log2 + 1):
        ...
        y_temp=y
        for r in range(resolution_log2, res-1, -1):
            y_temp = Conv2DLReq(y_temp.shape[1].value, None, normal_gain, nf(res-5), kernel_size=3, strides=(1,1), data_format='channels_first',padding='same', bias_initializer='zeros')(y_temp)
        ...
        x = Conv2DLReq(x.shape[1].value, None, normal_gain, nf(res-5), kernel_size=3, strides=(1,1), data_format='channels_first',padding='same', bias_initializer='zeros')(x)
        x = LeakyReLU()(x)
        x = x * tf.rsqrt(tf.reduce_mean(tf.square(x), axis=1, keepdims=True) + 1e-8) # Pixel Normalization
        ...
        img = Conv2DLReq(x.shape[1].value, None, 1 , num_channels, kernel_size=1, strides=(1,1), data_format='channels_first',padding='same', bias_initializer='zeros')(x)
...
def Generator_tf2(
    ...
    #lod_in = tf.cast(tf.get_variable('lod', initializer=np.float32(0.0), trainable=False), dtype)
    lod_in = tf.cast(tf.Variable(initial_value=np.float32(0.0), trainable=False), dtype)
    ...
    print(combo_in)
    #x = conv2d(combo_in, fmaps=nf(resolution_log2-1), kernel=1, use_wscale=use_wscale)#fromrgb(combo_in, resolution_log2)
    ...
    kernel=1
    fmaps=nf(resolution_log2-1)
    #w = get_weight([kernel, kernel, x.shape[1].value, fmaps], gain=gain, use_wscale=use_wscale)
    #w = tf.cast(w, x.dtype)
    #x = Conv2D(filters=3, strides=(1,1,1,1), padding='SAME', data_format='NCHW') # number for filters dimension
    x = Conv2D(3, kernel_size=1, strides=(1,1),  padding='same', bias_initializer='zeros')(latent)
    x = LeakyReLU()(x)
    y = x
    for res in range(resolution_log2, 4, -1):
        x = Conv2D(3, kernel_size=1, strides=(1,1),  padding='same', bias_initializer='zeros')(x)
        LeakyReLU()(x)
        #x= act(apply_bias(conv2d_downscale2d(x, fmaps=128, kernel=3, use_wscale=use_wscale)))

    Model(inputs=[latent], outputs=[x]).summary()
