...
def Generator_loss(G, D, reals, minibatch_size, opt=None, lod_in=0.0, training_set=None, cond_weight = 1.0, network_size=256): # Weight of the conditioning term.
    print('Mini-batch size G ' + str(minibatch_size))
...
def Discriminator_loss(G, D, reals, minibatch_size, opt, lod_in=0.0, training_set=None, labels=None,
    ...
    with tf.GradientTape(persistent=True) as grad_total_tape:
        ...
        with tf.GradientTape() as grad_penalty_tape:
            ...
            mixed_scores_out, mixed_labels_out = D([mixed_images_out, lod_in], training=True)
            #final mixed loss
